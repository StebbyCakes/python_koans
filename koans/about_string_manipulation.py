...
class AboutStringManipulation(Koan):
    ...
    # splits the words from the string into their own
    ...
    # splits the words from the string into their own string array
    ...
    def test_raw_strings_do_not_interpret_escape_characters(self):
        # every line counts for 1 len in a string
        string = r'\n'
        self.assertNotEqual('\n', string)
        self.assertEqual('\\n', string)
    ...
    def test_strings_can_be_joined(self):
        # .join puts all strings being called and makes them add into a single string
        words = ["Now", "is", "the", "time"]
        self.assertEqual("Now is the time", ' '.join(words))

    def test_strings_can_change_case(self):
        # Different methods for saying what to do to a string without having to manually change the string
